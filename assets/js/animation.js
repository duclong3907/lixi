...
document.addEventListener('click', function (e) {
    // Kiểm tra xem click có xảy ra ngoài khu vực lx_ctn không
    if (!lx_ctn.contains(e.target) && !opened.contains(e.target)) {
        // Đặt lại các thuộc tính style để "đóng" thiệp
        opened.style.display = "";
        napd.style.height = "";
        lx_write.style.transform = "";
        lx_write.style.height = "";
        lx_write.style.margin = "";
        lx_write.classList.remove('br12px');
        lx_form.style.display = "none";

        // Đặt lại chiều cao của napm sau khi "đóng" thiệp
        setTimeout(() => {
            napm.style.height = "0px";
        }, 500);
    }
});